...
pre.controller('adddalkcontroller', function ($scope, $http) 
{
	...
	$scope.edit_id=function(id)
	{
		...
		$http.post('edit_activity_type', { 'id':id}) 
	...
	$scope.del=function(id)
	{
		...
		{
			$scope.list=true;
			$http.post('delete_activity_type',{ 'id':id}) 
				.success(function (data, status, headers, config)
		{
				alert ("Data has been deleted Successfully");
				window.open("list_activities_type","_self");
		});
	...
	$scope.update_data= function()
	{
		...
		$http.post('update_activity_type', 
		{
				    'id':$scope.id,
					'name' :$scope.Name,
